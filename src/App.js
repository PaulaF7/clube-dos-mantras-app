 ...
 *
 * v2.0: Implementação da funcionalidade "Meu Santuário" para
 * gravação, importação e criação de playlists de áudios personalizados.
 *
 */
                    ...
                    {playlists.length > 0 ? (
                         playlists.map(playlist => (
                            <div key={playlist.id} className="glass-card !p-4 flex justify-between items-center">
                                ...
                                <div className="flex items-center gap-2">
                                    <button onClick={() => onStartPlaylist(playlist)} className="p-2 rounded-full hover:bg-white/10 transition-colors"><Play size={18} /></button>
...
const AudioCreatorModal = ({ isOpen, onClose }) => {
    ...
    const handleSave = async () => {
        ...
        try {
            const fileName = `${Date.now()}_${nome.replace(/\s+/g, '_')}.webm`;
                    ...
                    {status === 'recorded' && (
                        <div className="space-y-4">
                            ...
                            <div className="flex gap-4">
                                ...
                                <button onClick={() => setStatus('idle')} className="w-full btn-secondary">Descartar</button>
                            </div>
                        </div>
                    )}
...
/**
 * Player para áudios personalizados e playlists.
 */
